...
const jobRole = () =>{
  ...
  document.getElementsByClassName('bignamepost')[0].innerHTML = event.target.value
}
const summary = () =>{
...
function setEducation(obj){
  html =`  <div class="edcution-content"><button class="edit-button" onclick="editEducation()"><i class="fa fa-pencil-square-o" aria-hidden="true" data-html2canvas-ignore="true"></i></button>
  <button class="right-button" onclick="rightEducation()"><i class="fa fa-check" data-html2canvas-ignore="true"></i></button>
  <i class="fa fa-trash-o skillDelete" onclick="removesection()" data-html2canvas-ignore="true"></i>
  <p class='edit'>
      <i class="fa fa-institution" aria-hidden="true"></i>&nbsp;&nbsp; <span id="editUniversity"> ${obj.university}</span>
    </p>
    <p>
      <i class="fa fa-mortar-board" aria-hidden="true"></i>&nbsp;&nbsp; <span id="editDegree">${obj.degree}</span>
    </p>
</div>`
  ...
  function submitskill(){
   ...
   let points = document.getElementsByClassName("skill_points")
    for (const point of points) {
      document.getElementById("skill-container").insertAdjacentHTML("beforeend",`<li>${point.value}</li>`)
    }
...
function setExperince(data){
  ...
  <i class="fa fa-pencil-square-o exp-position btn" aria-hidden="true" onclick="editExperience()" data-html2canvas-ignore="true"></i>
  <i class="fa fa-trash-o delete " onclick="removesection()" data-html2canvas-ignore="true"></i>
  <i class="fa fa-check exp-position btn"  onclick="rightExpeince()" id="check" data-html2canvas-ignore="true"></i>          
  <p class="company">${data.company},<span class="comp-post">${data.post}</span></p>
  <p>${data.startYear}-${data.endYear}</p>
<ul class="experince_container">
${li}
</ul>
</div>`

document.getElementsByClassName("exp-container")[0].insertAdjacentHTML("beforeend",html)
}
...
function submitWork(){
  ...
  <fieldset>
  <legend><span class="number">3</span> Work Experince</legend>
...
<option value="2021">2021</option>
 ...
 <div class="button-container">
  ...
  university =  parent.querySelector("#editUniversity")
 degree=  parent.querySelector("#editDegree")
university.setAttribute("contenteditable","true")
degree.setAttribute("contenteditable","true")
university.style.borderBottom = "1px solid"
degree.style.borderBottom = "1px solid"
event.target.style.display="none"
event.target.parentElement.parentElement.querySelector(".right-button").style.display = "block"
...
function rightEducation(){
  ...
  university =  parent.querySelector("#editUniversity")
 degree=  parent.querySelector("#editDegree")
university.setAttribute("contenteditable","false")
degree.setAttribute("contenteditable","false")
university.style.borderBottom = "0"
degree.style.borderBottom = "0"
event.target.style.display="none"
btn = event.target.parentElement.parentElement.querySelector(".edit-button").style.display = "block "
...
child =element.querySelectorAll("*")
for (const ele of child) {
  ele.style.borderBottom = 0
} 
btn =element.querySelectorAll(".btn")
for (const ele of btn ) {
  ele.style.display = "none"
}

let opt = {
  margin:  [0 ,0 , -1 ,0],  //top, left, buttom, right,
